...
### Fixed
- **Scope-resolution wedge on large monorepos (#1741)** — `runScopeResolution`, `resolveReferenceSites`, and the receiver-bound / free-call / via-lookup emit passes were synchronous. For openclaw-class workspaces (~16k TS/JS files, ~5M reference sites per language) they processed the whole language in a single uninterrupted stack frame, blocking the event loop and preventing V8 from running incremental mark-compact against the multi-GB working set. User-visible symptom: "frozen IO + spinning CPU + RSS plateau" at ~5.2 GB after `[deferred-profile] buildHeritageMap: skipped` with no `.gitnexus/` artifact produced. These functions now yield cooperatively at phase boundaries and inside their hot loops, and the per-file content snapshot is released at the seam between the populate band and the resolve/emit band (~80 MB recovered). Sister fix to #1800 (which addressed cross-language pressure in the same wedge class for PHP).
