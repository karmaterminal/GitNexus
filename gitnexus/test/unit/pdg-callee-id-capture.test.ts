...
describe('callee-id capture — emitReferencesViaLookup', () => {
  it('captures a CALLS emitted via the inline addRelationship', async () => {
    ...
    const acc = createCalleeIdAccumulator();
    const result = await emitReferencesViaLookup(
      graph,
      indexes,
      referenceIndex,
      lookup,
  ...
  it('R4 gating — sink undefined: identical edges, nothing captured', async () => {
    const callerFn = def('def:caller', 'Function', 'caller');
...
describe('callee-id capture — emitFreeCallFallback (inline addRelationship)', () => {
  ...
  it('captures the resolved callee id at the free-call site', async () => {
    const { graph, indexes, lookup } = buildDriver();
    const model = createSemanticModel();
    const workspaceIndex = buildWorkspaceResolutionIndex([parsed]);
    const acc = createCalleeIdAccumulator();

    const emitted = await emitFreeCallFallback(
      graph,
      indexes,
      [parsed],
      lookup,
  ...
  it('R4 gating — sink undefined: same CALLS edge, nothing captured', async () => {
